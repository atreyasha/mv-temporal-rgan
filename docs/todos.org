...
***** TODO find error leading to mnist low intensity and final row high intensity, perhaps upsample in generator above dimensionality and then downsample (or more precise discriminator)
...
***** TODO update RGAN version 3 with performance summary and manage branches, add extra option to ignore pics/gifs when cloning unless prompted
