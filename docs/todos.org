...
***** TODO extend to auxiliary architecture, where we can also use the discriminator for other tasks (would be interesting for supervised workflow)
***** TODO change adam beta1 coefficient to 0.5 and check differences in results, it reduces weight decay over time
