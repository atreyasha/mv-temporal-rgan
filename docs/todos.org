...
***** TODO add citation for spectral normalization code
...
***** TODO make modular function for model restoration
***** TODO port code to tensorflow2 for better integration -> might solve problem with accuracy printing based on non-binary target labels
***** TODO choose best model and extend to RCGAN, look up ganhacks for further possible improvements such as adding leaky-relu everywhere; leave best model training much longer on faces to check for better convergence
