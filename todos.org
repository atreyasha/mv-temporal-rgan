...
*** Model visualization
***** TODO use plotnine/ggplot2 to add graphs for loss with time given different combinations/model-types
***** add to readme that gif-progress must be installed on system
***** add epoch numbers/status bar to gifs for better visualization
***** add quick routine for visualization which can benefit investigations later on
