...
class RCGAN():
    ...
    def train(self, data, direct, plot_samples=5):
        ...
        fieldnames = [
            "epoch", "batch", "d_loss", "d_a_loss", "g_loss", "g_a_loss"
        ]
        ...
        constant_noise = np.concatenate(np.repeat(constant_noise[None, :],
                                                  ...
                                                  axis=0),
                                        axis=0)
        constant_labels = np.repeat(np.arange(0, self.num_classes),
                                    plot_samples)
        np.random.seed(None)
        ...
        for epoch in range(self.epochs):
            ...
            real_labels = np.clip(
                np.random.normal(loc=0.90,
                                 scale=0.005,
                                 size=(self.batch_size, 1)), None, 1)
            for batch in range(runs):
                ...
                d_loss_real = self.discriminator.train_on_batch(
                    real_imgs, [real_labels, img_labels])
                d_loss_fake = self.discriminator.train_on_batch(
                    fake_imgs, [fake_labels, sampled_img_labels])
                d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
                # train generator while freezing discriminator
                g_loss = self.combined.train_on_batch(
                    [noise, sampled_img_labels],
                    [real_labels, sampled_img_labels])
                # plot the progress
                if (batch + 1) % 20 == 0:
                    print(
                        "epoch: %d [batch: %d] [D loss: %f, D.A Loss: %f] [G loss: %f, G.A Loss: %f]"
            ...
            test_img = self.generator.predict(
                [constant_noise, constant_labels])
