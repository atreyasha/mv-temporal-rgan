...
plot_evolution <- function(direct, ideal_ticks = 10) {
  ...
  g <- ggplot() +
    ...
    xlab("\nEpoch") +
    ylab("Batch-Averaged Cross Entropy Loss\n") +
    theme_bw() +
    theme(
      text = element_text(size = 13, family = "CM Roman"),
  ...
  if ("until" %in% names(init_df)) {
    ...
    g <- g +
      {
        if (plot_breaks) geom_rect(data = breaks, aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = "Missing data"), alpha = 0.4)
      } +
      {
...
parser <- add_option(parser, c("-t", "--ticks"),
