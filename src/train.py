...
def singularTrain(model_name, data, latent_dim, epochs, batch_size,
                  learning_rate, g_factor, droprate, momentum, alpha,
                  saving_rate):
    """
    ...
    if model_name == "RGAN":
        ...
        model = RCGAN(num_classes, latent_dim, im_dim, epochs, batch_size,
                      learning_rate, g_factor, droprate, momentum, alpha,
                      saving_rate)
...
def continueTrain(direct, arguments):
    ...
    if "_" not in re.sub(r".*_R(C)?GAN_", "", direct):
        log_dir = re.sub(r"(R(C)?GAN_)", "\g<1>" + getCurrentTime() + "_",
                         directLong)
        log_dir_pass = re.sub("./pickles/", "", log_dir)
    else:
        temp = re.sub(r"(.*)(_R(C)?GAN_)(.*)(_(.*)$)", "\g<4>\g<2>\g<6>",
                      direct)
        log_dir_pass = re.sub(r"(R(C)?GAN_)", "\g<1>" + getCurrentTime() + "_",
                              temp)
    ...
    model = restore_model(model, train_set, model_name, directLong,
                          log_dir_pass)
    # resume training
...
if __name__ == "__main__":
    ...
    parser.add_argument("--saving-rate",
                        type=int,
                        default=10,
    ...
    if args.plot_model:
        ...
        todel = [
            i for i in range(len(arguments)) if arguments[i] == "--log-dir"
        ]
        for i in sorted(todel, reverse=True):
            del arguments[i:i + 2]
        arguments = [
            re.sub("-", "_", re.sub("--", "", arguments[i])) if i %
            2 == 0 else arguments[i] for i in range(len(arguments))
        ]
        ...
        singularTrain(args.model, args.data, args.latent_dim, args.epochs,
                      ...
                      args.droprate, args.momentum, args.alpha,
                      args.saving_rate)
